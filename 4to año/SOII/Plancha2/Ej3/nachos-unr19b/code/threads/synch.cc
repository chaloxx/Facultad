...
Lock::Acquire()
{
  ...
  DEBUG('s',"%s intentando tomar lock %s \n",currentThread-> GetName(), name);
  s->P(); // Tomar lock
...
Lock::Release()
{
  DEBUG('s',"Hilo %s intentando tomar lock %s \n",currentThread-> GetName(), name);
...
{
  if (lockThread == currentThread)// Podemos identificar a cada hilo por su dirección de memoria
  {
    return true;
  }
  return false;
...
Condition::Wait()
{
  s = new Semaphore(name,0); // Creamos un semáforo para poner a dormir al hilo hasta que llega la signal
