module ListSeq (filterS,scanS,singletonS, reduceS,appendS,tabulateS, nthS,lengthS,contractS,takeS,dropS) where
...
instance Seq [] where
    ...
    expandS f [] [] = []
    expandS f (x:[]) (z:zs) = [z]
    expandS f (x:y:xs) (z:zs) = let (x',xs') = (f z x) ||| (expandS f xs zs)
                               in z:x':xs'
